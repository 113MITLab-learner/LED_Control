...
#define EXAMPLE_ESP_WIFI_SSID      "Daniel iPhone"
#define EXAMPLE_ESP_WIFI_PASS      "pinghsiang"
...
static esp_err_t hello_get_handler(httpd_req_t *req)
{
    ...
    const char* resp_str = "<!DOCTYPE html>"
                           ...
                           "<button onclick=\"toggleLED('on')\">開啟 LED</button>"
                           "<button onclick=\"toggleLED('off')\">關閉 LED</button>"
                           "<p>LED 狀態: <span id=\"ledStatus\">未知</span></p>"
                           "<script>"
                           "function toggleLED(state) {"
                           "  fetch('/led?state=' + state)"
                           "    .then(response => response.json())"
                           "    .then(data => {"
                           "      document.getElementById('ledStatus').innerText = data.led_state ? '開啟' : '關閉';"
                           "    });"
                           "}"
                           // 在頁面加載時取得 LED 狀態
                           "window.onload = function() {"
                           "  fetch('/led?state=status')"
                           "    .then(response => response.json())"
                           "    .then(data => {"
                           "      document.getElementById('ledStatus').innerText = data.led_state ? '開啟' : '關閉';"
                           "    });"
...
static esp_err_t led_control_get_handler(httpd_req_t *req)
{
    ...
    if (strcmp(action, "on") == 0) {
        ...
        led_status = 1; // 更新狀態
        ESP_LOGI(TAG, "LED 已開啟");
    } else if (strcmp(action, "off") == 0) {
        gpio_set_level(LED_GPIO, 0);
        led_status = 0;
        ESP_LOGI(TAG, "LED 已關閉");
    }

    char resp_str[64];
    snprintf(resp_str, sizeof(resp_str), "{\"led_state\": %d}", led_status);
...
void app_main(void)
{
    ...
    gpio_set_level(LED_GPIO, 0); // 初始為關閉狀態
